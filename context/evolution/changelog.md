...
### Added
- **2026-08-31** — **Confiabilidade sobre UDP** (PR #3, ADR-0007): tipo `NACK` + retransmissão sob demanda (origem reenvia DATA; quem conhece reenvia ACK) + thread periódica que reenvia DATA/ACK pendentes e pede o que falta. Recupera perda de DATA **e** de ACK. Validado com perda injetada de 30% e 50% (ordem global idêntica). Testes `teste_retransmissao_data_perdida`/`teste_retransmissao_ack_perdido` + orquestrador `DROP_PROB`. Pattern `reliable-multicast-nack-retransmission`.
